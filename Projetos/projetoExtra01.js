...
voteCount = (userVote) => {
	if (userVote == `1`) {
		...
		voteNull++;
		console.log(`\nSeu voto foi computado ${candidates[3]['Candidato']} `);
	} else if (userVote == `5`) {
		voteBlank++;
		console.log(`\nSeu voto foi em ${candidates[4]['Candidato']}`);
	} else {
		console.log(`\nSeu voto não será computado. Digite um número de acordo com as opções. `);
	}
};
...
const candidates = [
	{ Opção: 1, Candidato: 'Leo' },
	{ Opção: 2, Candidato: 'Duda' },
...
//LAÇO PARA REPETIR O PROGRAMA.
while (exit != `nao` && exit != `n`) {
	...
	if ((userAge > 15 && userAge < 18) || userAge > 70) {
      ...
      confirmationVote = prompt (`Confirma [S] ou [N]? `);

      console.log();

			console.clear();

      } while (confirmationVote != `sim` && confirmationVote != `s`)
        
		}

		console.log(`${userName}, obrigado por participar da eleição.\n`);
	...
	exit = prompt(`Outro eleitor irá votar? `).toLowerCase();

	console.clear();
}

console.log(`O resultado das eleições foi: \n`);
